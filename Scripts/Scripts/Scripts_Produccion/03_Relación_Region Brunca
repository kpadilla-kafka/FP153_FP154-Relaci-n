/* =========================================================
   ...
   Objetivo:
   Crear 18 tareas nuevas en FP154 para el usuario indicado
   y relacionarlas con su registro existente en FP153.

   Alcance:
   6 Áreas Rectoras de Salud x 3 roles = 18 relaciones.

   Usuarios PROD:
   - ID 8  : Rosibel Cascante Fallas
   - ID 9  : Ronny Fonseca Fallas
   - ID 10 : Michael Mora Fernandez

   Resultado esperado por ejecución:
   - FP154 debe incrementar en 18 registros.
   - FP153 del usuario debe mostrar 18 ramas asociadas.
...
DECLARE @IdUsuario153 INT = 8; -- Rosibel Cascante Fallas
-- Para Ronny:   DECLARE @IdUsuario153 INT = 9;  -- Ronny Fonseca Fallas
-- Para Michael: DECLARE @IdUsuario153 INT = 10; -- Michael Mora Fernandez
...
IF (SELECT COUNT(*) FROM @RamasBrunca) <> 18
    THROW 50001, 'La cantidad de ramas Brunca no es 18.', 1;

-- 1. Crear 18 tareas nuevas en FP154 para el usuario
...
-- 2. Relacionar esas 18 tareas nuevas con el usuario FP153
